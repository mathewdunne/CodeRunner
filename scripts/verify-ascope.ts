...
async function verifyStagedBundle(): Promise<void> {
	assert(
		...
		"dist/advantagescope/index.html is missing. Run bun run build:ascope.",
	);
	assert(
	...
	assert(
		!/\b(?:src|href)="\//u.test(indexHtml),
		"AS Lite index.html contains root-absolute asset references.",
	);

	const mainBundle = await readFile(
		resolve(distDir, "bundles", "main.js"),
		"utf8",
	);
	const hubBundle = await readFile(
		resolve(distDir, "bundles", "hub.js"),
		"utf8",
	);
	assert(
		mainBundle.includes("frc-sim:set-nt4-endpoint"),
		"AS Lite main bundle is missing postMessage config support.",
	);
	assert(
		mainBundle.includes("frc-sim:nt4-endpoint-ready"),
		"AS Lite main bundle is missing endpoint acknowledgement.",
	);
	assert(
	...
	assert(
		hubBundle.includes("frcSimNt4Endpoint"),
		"AS Lite hub bundle is missing injected NT4 endpoint support.",
	);
	assert(
		hubBundle.includes("websocketUrl"),
		"AS Lite hub bundle is missing injected WebSocket URL support.",
	);
	assert(
		hubBundle.includes("aliveUrl"),
		"AS Lite hub bundle is missing injected alive URL support.",
	);
}
...
async function verifyScopeServing(): Promise<void> {
	...
	try {
		...
		assert(
			...
			"/scope/ did not serve HTML.",
		);

		const main = await app.fetch(
			new Request("http://localhost/scope/bundles/main.js"),
		);
		assert(
			main.status === 200,
			`/scope/bundles/main.js returned ${main.status}.`,
		);
		assert(
			(main.headers.get("content-type") ?? "").includes("text/javascript"),
			"main.js content type is wrong.",
		);

		const assets = await app.fetch(
		...
		assert(
			...
			"/scope/assets did not return an asset manifest.",
		);

		const redirect = await app.fetch(
			new Request("http://localhost/scope/www/www/textures/example.png"),
		);
		assert(
			redirect.status === 302,
			`/scope/www/www/* redirect returned ${redirect.status}.`,
		);
		assert(
