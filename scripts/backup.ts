...
function parseArgs(): Args {
	...
	for (let i = 0; i < args.length; i++) {
		if (args[i] === "--data-dir" && args[i + 1]) {
			...
			console.log(
				"Usage: bun scripts/backup.ts [--data-dir <path>] [--output <path>] [--projects-only]",
			);
			console.log("");
			console.log(
			...
			console.log(
				"  --output <path>     Output directory (default: <data-dir>/backups/YYYY-MM-DD-HHmmss/)",
			);
			console.log(
				"  --projects-only     Back up only per-workspace project files (legacy mode)",
			);
			process.exit(0);
		}
	}
		...
		for (const workspaceId of workspaceIds) {
			...
			if (hasProject) {
				try {
					await runTar([
						"-czf",
						resolve(workspaceBackupDir, "project.tar.gz"),
						"-C",
						projectPath,
						".",
					]);
			...
			if (hasAssets) {
				try {
					await runTar([
						"-czf",
						resolve(workspaceBackupDir, "assets.tar.gz"),
						"-C",
						assetsPath,
						".",
					]);
