...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "add": {
      ...
      console.log(`Allowlist now has ${result.emails.length} email(s) and ${result.domains.length} domain(s).`);
    ...
    case "remove": {
      if (!value) {
        console.error("Usage: bun scripts/allowlist.ts remove <email-or-domain>");
        process.exit(1);
      }
      const kind = isLikelyDomain(value) ? "domain" : "email";
    ...
    default:
      console.error("Usage: bun scripts/allowlist.ts <list|add|remove> [email-or-domain]");
      process.exit(1);
  }
}
