...
const dataDir = process.env.FRC_DATA_DIR ?? resolve(import.meta.dir, "..", "data");
const dbPath = process.env.FRC_DB_PATH ?? resolve(dataDir, "app.db");

const db = new Database(dbPath, { create: false });

const countResult = db.query("SELECT COUNT(*) AS count FROM audit_log WHERE occurred_at < ?").get(beforeMs) as { count: number };

if (dryRun) {
  console.log(`[dry-run] Would delete ${countResult.count} audit log entries before ${beforeDate}.`);
} else {
  db.query("DELETE FROM audit_log WHERE occurred_at < ?").run(beforeMs);
  console.log(`Deleted ${countResult.count} audit log entries before ${beforeDate}.`);
}
