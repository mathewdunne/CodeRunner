...
## Corpus Check
- 158 files · ~92,253 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2203 nodes · 3059 edges · 183 communities (171 shown, 12 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 20 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `12662cf9`
...
6. `createApp()` - 21 edges
7. `FRC Web Simulator V2: Design Document` - 20 edges
8. `FRC Web Simulator V2: Design Document` - 20 edges
9. `Robot` - 17 edges
10. `RobotContainer` - 15 edges
...
Cohesion: 0.07
Nodes (31): codeContainerName(), CodeContainerStatus, containerName(), ContainerOrchestrator, ContainerOrchestratorOptions, containerRuntimeState(), defaultDockerRunner(), dockerError() (+23 more)
...
Cohesion: 0.05
Nodes (46): acceptWebSocket(), ActiveRun, activeRuns, asWebSocket(), collectProcess(), delay(), docker(), dockerExec() (+38 more)
...
Cohesion: 0.07
Nodes (19): AppSocket, AssetManifest, BunUpgradeServer, createProjectArchive(), HOP_BY_HOP_HEADERS, LspSocketData, lspWebSocketResponse(), Nt4SocketData (+11 more)

### Community 12 - "Community 12"
Cohesion: 0.14
Nodes (24): ControlAppOptions, DockerCommandResult, RunCommandFactory, createAdvantageScopeDist(), createFakeDocker(), createTemplate(), createWebDist(), FakeContainer (+16 more)
...
Cohesion: 0.11
Nodes (18): 1. Bootstrap shadcn first, 2. Path aliases and config cleanup, 3. Install initial primitives, 4. Decompose `main.tsx`, 5. State extraction, 6. Style cleanup, 7. Editor iframe — passthrough, 8. AS Lite iframe — preserve handshake (+10 more)
...
Cohesion: 0.16
Nodes (18): 9. Common Failures and Recovery, AS Lite not showing telemetry, Build queue saturated, Code container OOM, code:bash (# Container is auto-recreated on next run. If persistent:), code:bash (# Restart the code container), code:bash (# 1. Prune run logs (safest, usually largest)), code:bash (bun run dev:control) (+10 more)
...
Cohesion: 0.15
Nodes (13): consumeLines(), defaultRunCommandFactory(), dockerRunScript(), lineLooksReady(), randomRunId(), RunCommand, RunCommandContext, RunConnection (+5 more)
...
Cohesion: 0.18
Nodes (11): DockerRunner, missing(), waitFor(), aliceCookie, dockerRunner(), fakeDocker, receivedHeaders, result (+3 more)
...
Cohesion: 0.22
Nodes (10): 10. Host Sizing, code:bash (bun run measure), code:block38 (═══ FRC Web Simulator — Resource Report ═══), code:bash (CODE_MEMORY_LIMIT=2048m), code:bash (CODE_MEMORY_LIMIT=3072m), code:bash (CODE_MEMORY_LIMIT=3072m), Measuring actual usage, Per-student resource usage (+2 more)

### Community 82 - "Community 82"
Cohesion: 0.22
Nodes (10): 3. Starting the App, code:bash (curl http://localhost:4000/admin/status), code:bash (bun run docker:cleanup), code:bash (# Stop all workspaces), code:bash (docker stop $(docker ps -q --filter label=frc-sim.managed=tr), code:bash (bun run dev:control), One-command start, Stop containers too (+2 more)
...
Cohesion: 0.25
Nodes (9): 6. Backup and Restore, code:bash (bun run backup -- --output /path/to/backup), code:bash (bun run restore -- <backup-dir>), code:bash (bun run restore -- <backup-dir> --dry-run), code:bash (bun run restore -- <backup-dir> --workspace ws_abc123...), Create a backup, Recommended backup schedule, Restore from backup (+1 more)
...
Cohesion: 0.32
Nodes (8): 8. Monitoring, Admin status API, code:bash (bun run measure), code:bash (bun run measure -- --json), code:bash (# All managed containers), Docker container status, Resource measurement, Watch for issues

### Community 98 - "Community 98"
Cohesion: 0.29
Nodes (8): 5. Configuration, code:bash (CODE_MEMORY_LIMIT=2048m), code:bash (CODE_MEMORY_LIMIT=2560m), code:bash (bun run backup), Environment variables, Key settings for classroom use, Tuning for constrained hosts, Tuning for large classrooms
...
Cohesion: 0.67
Nodes (3): isReservedProjectTempName(), projectTreeResponse(), readProjectTreeNode()

## Knowledge Gaps
- **1012 isolated node(s):** `workspaceSlugSchema`, `userIdSchema`, `workspaceIdSchema`, `sessionIdSchema`, `displayNameSchema` (+1007 more)
...
- **Why does `isWorkspaceSlug()` connect `Community 9` to `Community 6`, `Community 79`?**
