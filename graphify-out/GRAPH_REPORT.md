...
## Corpus Check
- 210 files · ~104,945 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2433 nodes · 3482 edges · 202 communities (188 shown, 14 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 21 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `623c3208`
...
1. `ContainerOrchestrator` - 53 edges
2. `cn()` - 51 edges
3. `AppStorage` - 36 edges
...
8. `FRC Web Simulator V2: Design Document` - 20 edges
...
Cohesion: 0.06
Nodes (32): codeContainerName(), CodeContainerStatus, containerName(), ContainerOrchestrator, ContainerOrchestratorOptions, containerRuntimeState(), defaultDockerRunner(), dockerError() (+24 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (39): ControlConfig, ControlConfigInput, defaultContainerUser(), defaultDataDir, defaultHalsimPortRange, defaultLspPortRange, defaultSimPortRange, defaultVscodePortRange (+31 more)
...
Cohesion: 0.04
Nodes (47): AdminActionResponse, adminActionResponseSchema, AdminStatusResponse, adminStatusResponseSchema, AdminWorkspaceStatus, adminWorkspaceStatusSchema, ContainerRole, ContainersStatusResponse (+39 more)
...
Cohesion: 0.11
Nodes (25): EditorPane(), EditorPaneProps, RunControls(), RunControlsProps, ScopePane, ScopePaneProps, pillVariant(), StatusStrip() (+17 more)

### Community 10 - "Community 10"
Cohesion: 0.07
Nodes (23): AppSocket, AssetManifest, BunUpgradeServer, createProjectArchive(), HalSimSocketData, HOP_BY_HOP_HEADERS, isReservedProjectTempName(), LspSocketData (+15 more)

### Community 11 - "Community 11"
Cohesion: 0.1
Nodes (23): ConsolePane(), ConsolePaneProps, IDELayout(), IDELayoutProps, cn(), Card(), CardAction(), CardContent() (+15 more)
...
Cohesion: 0.14
Nodes (24): ControlAppOptions, createApp(), DockerRunner, createAdvantageScopeDist(), createTemplate(), createWebDist(), FakeContainer, FakeContainerPort (+16 more)
...
Cohesion: 0.13
Nodes (18): ConsoleLine(), ConsolePanel(), ConsolePanelProps, ConsoleTab, parseConsoleLine(), TAG_CLASSES, MODE_ACTIVE_CLASSES, MODE_LABELS (+10 more)

### Community 20 - "Community 20"
Cohesion: 0.1
Nodes (10): JavaLspController, canOpen(), fileName(), languageFor(), LspStatus, ModelState, onPageHide(), OpenFile (+2 more)
...
Cohesion: 0.12
Nodes (16): MatchTimer(), MatchTimerProps, STATION_LABELS, STATIONS, DropdownMenu(), DropdownMenuCheckboxItem(), DropdownMenuContent(), DropdownMenuItem() (+8 more)
...
Cohesion: 0.16
Nodes (18): 9. Common Failures and Recovery, AS Lite not showing telemetry, Build queue saturated, Code container OOM, code:bash (# Restart the code container), code:bash (# 1. Prune run logs (safest, usually largest)), code:bash (bun run dev:control), code:bash (bun run migrate:status  # verify DB is accessible) (+10 more)
...
Cohesion: 0.13
Nodes (17): 2.1 Clone and initialize, 2.2 Build container images, 2.2 Build the code container image, 2.3 Build web assets, 2.4 Configure environment, 2.5 Run migrations, 2.6 Verify the setup, 2.7 Measure host resources (+9 more)
...
Cohesion: 0.13
Nodes (14): aliceWorkspace, backedUpProject, backupsDir, bobWorkspace, body, cookie, fakeDocker, leaseAfter (+6 more)
...
Cohesion: 0.22
Nodes (10): SimControls(), SimControlsProps, LED_CLASSES, LedColor, StatusReadout(), StatusReadoutProps, Tooltip(), TooltipContent() (+2 more)

### Community 57 - "Community 57"
Cohesion: 0.21
Nodes (9): ALLIANCE_LABELS, DriverStation(), DriverStationProps, MODE_LABELS, MODE_PILL_CLASSES, JoystickPanel(), Badge(), badgeVariants (+1 more)
...
Cohesion: 0.17
Nodes (12): consumeLines(), defaultRunCommandFactory(), dockerRunScript(), lineLooksReady(), randomRunId(), RunCommand, RunCommandContext, RunCommandFactory (+4 more)

### Community 65 - "Community 65"
Cohesion: 0.21
Nodes (13): 7. Cache Cleanup, code:bash (bun run docker:cleanup), code:bash (bun run docker:cleanup -- --dry-run), code:bash (rm -rf data/users/<workspaceId>/home/), code:bash (rm -rf data/users/<workspaceId>/logs/runs/*), code:bash (for dir in data/users/*/; do), code:bash (docker system prune -f          # Remove unused containers, ), code:bash (curl http://localhost:4000/admin/status | jq .) (+5 more)
...
Cohesion: 0.2
Nodes (10): isWorkspaceSlug(), App(), EditorStatus, fetchJson(), LoadState, loadWorkspace(), rootElement, RunStatus (+2 more)

### Community 80 - "Community 80"
Cohesion: 0.2
Nodes (10): createFakeDocker(), missing(), aliceCookie, dockerRunner(), fakeDocker, receivedHeaders, result, source (+2 more)
...
Cohesion: 0.24
Nodes (5): ThemeProvider(), useTheme(), WorkspaceLayout(), router, Toaster()
...
Cohesion: 0.22
Nodes (10): 5. Configuration, code:bash (# Stop all workspaces), code:bash (docker stop $(docker ps -q --filter label=frc-sim.managed=tr), code:bash (CODE_MEMORY_LIMIT=2048m), code:bash (CODE_MEMORY_LIMIT=2560m), Environment variables, Key settings for classroom use, Stop containers too (+2 more)
...
Cohesion: 0.22
Nodes (8): aliceCookie, cookie, sessionCount, userCount, workspace, workspaceCount, cookieFrom(), exists()

### Community 100 - "Community 100"
Cohesion: 0.33
Nodes (8): 1. Prerequisites, 4. Stopping the App, Between class sessions, FRC Web Simulator V1 — Operator Runbook, FRC Web Simulator V2 — Operator Runbook, Graceful stop, Quick Reference Card, Table of Contents

### Community 101 - "Community 101"
Cohesion: 0.28
Nodes (9): 8. Monitoring, Admin status API, code:bash (bun run measure), code:bash (bun run measure -- --json), code:bash (# All managed containers), code:bash (# Container is auto-recreated on next run. If persistent:), Docker container status, Resource measurement (+1 more)

### Community 102 - "Community 102"
Cohesion: 0.25
Nodes (9): 10. Host Sizing, code:block38 (═══ FRC Web Simulator — Resource Report ═══), code:bash (CODE_MEMORY_LIMIT=2048m), code:bash (CODE_MEMORY_LIMIT=3072m), code:bash (CODE_MEMORY_LIMIT=3072m), Measuring actual usage, Per-student resource usage, Sizing recommendations (+1 more)
...
Cohesion: 0.33
Nodes (7): 3. Starting the App, code:bash (curl http://localhost:4000/admin/status), code:bash (bun run docker:cleanup), code:bash (bun run dev:control), One-command start, Verify the app is running, What happens on first student login
...
Cohesion: 0.29
Nodes (7): halsimWebSocketResponse(), nt4WebSocketResponse(), probeVscodeReady(), requestedProtocols(), stripHopByHopHeaders(), vscodeHttpProxyResponse(), vscodeWebSocketResponse()
...
## Knowledge Gaps
- **1067 isolated node(s):** `workspaceSlugSchema`, `userIdSchema`, `workspaceIdSchema`, `sessionIdSchema`, `displayNameSchema` (+1062 more)
...
- **Why does `isWorkspaceSlug()` connect `Community 79` to `Community 88`, `Community 6`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `App()` connect `Community 79` to `Community 3`, `Community 20`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `workspaceSlugFromLocation()` connect `Community 79` to `Community 20`?**
