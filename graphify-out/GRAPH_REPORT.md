...
## Corpus Check
- 200 files · ~100,455 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2357 nodes · 3327 edges · 195 communities (182 shown, 13 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 21 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b12b4537`
...
Cohesion: 0.06
Nodes (33): codeContainerName(), CodeContainerStatus, containerName(), ContainerOrchestrator, ContainerOrchestratorOptions, containerRuntimeState(), defaultDockerRunner(), dockerError() (+25 more)
...
Cohesion: 0.11
Nodes (31): ControlAppOptions, DockerCommandResult, aliceCookie, cookie, sessionCount, userCount, workspace, workspaceCount (+23 more)
...
Cohesion: 0.09
Nodes (13): ResolvedTheme, Theme, THEME_VALUES, ThemeProvider(), ThemeProviderContext, ThemeProviderProps, ThemeProviderState, useTheme() (+5 more)
...
Cohesion: 0.15
Nodes (15): RunControls(), RunControlsProps, ScopePane, ScopePaneProps, pillVariant(), StatusStrip(), StatusStripProps, Topbar() (+7 more)
...
Cohesion: 0.15
Nodes (19): 9. Common Failures and Recovery, AS Lite not showing telemetry, Build queue saturated, Code container OOM, code:bash (# Container is auto-recreated on next run. If persistent:), code:bash (# Restart the code container), code:bash (# 1. Prune run logs (safest, usually largest)), code:bash (bun run dev:control) (+11 more)
...
Cohesion: 0.21
Nodes (10): EditorPane(), EditorPaneProps, useContainerStatus(), EditorStatus, useEditorReachability(), useRunChannel(), useScopeHandshake(), LoadState (+2 more)
...
Cohesion: 0.17
Nodes (13): 5. Configuration, code:bash (bun run docker:cleanup), code:bash (# Stop all workspaces), code:bash (docker stop $(docker ps -q --filter label=frc-sim.managed=tr), code:bash (CODE_MEMORY_LIMIT=2048m), code:bash (CODE_MEMORY_LIMIT=2560m), code:bash (bun run backup), Environment variables (+5 more)
...
Cohesion: 0.2
Nodes (10): missing(), waitFor(), aliceCookie, dockerRunner(), fakeDocker, receivedHeaders, result, source (+2 more)
...
Cohesion: 0.22
Nodes (11): 8. Monitoring, Admin status API, code:bash (docker system prune -f          # Remove unused containers, ), code:bash (curl http://localhost:4000/admin/status | jq .), code:bash (bun run measure), code:bash (bun run measure -- --json), code:bash (# All managed containers), Docker container status (+3 more)
...
Cohesion: 0.27
Nodes (10): 7. Cache Cleanup, code:bash (bun run docker:cleanup), code:bash (bun run docker:cleanup -- --dry-run), code:bash (rm -rf data/users/<workspaceId>/home/), code:bash (rm -rf data/users/<workspaceId>/logs/runs/*), code:bash (for dir in data/users/*/; do), Prune all regenerable data, Prune Gradle/tool caches (+2 more)
...
Cohesion: 0.4
Nodes (4): ConsolePane(), ConsolePaneProps, ScrollArea(), ScrollBar()
...
## Knowledge Gaps
- **1031 isolated node(s):** `workspaceSlugSchema`, `userIdSchema`, `workspaceIdSchema`, `sessionIdSchema`, `displayNameSchema` (+1026 more)
...
- **Why does `isWorkspaceSlug()` connect `Community 9` to `Community 93`, `Community 6`?**
