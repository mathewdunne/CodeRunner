...
describe("useAutoChoosers", () => {
	...
	test("fetches and parses on mount", async () => {
		...
		expect(fetch).toHaveBeenCalledWith(
			"/u/my-slug/api/sim/auto-choosers",
	...
	test("selectAuto sends PATCH, updates local status", async () => {
		...
		await act(async () => {
			await result.current.selectAuto({
				key: "/SmartDashboard/Auto",
				selected: "Right",
			});
		});
		...
		const patchCall = (fetch as ReturnType<typeof vi.fn>).mock.calls.find(
			(c: unknown[]) =>
				(c[0] as string).includes("/api/sim/auto-chooser") &&
		...
		vi.stubGlobal(
			...
			vi.fn().mockImplementation((url: string, opts?: RequestInit) => {
				if (url.includes("/api/sim/auto-chooser") && opts?.method === "PATCH") {
					return Promise.resolve({
						ok: false,
						json: () => Promise.resolve({ error: "NT4 not connected" }),
					});
				}
				return Promise.resolve({
					ok: true,
