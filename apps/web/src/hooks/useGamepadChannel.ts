...
import { gamepadServerMessageSchema, type GamepadState } from "@/lib/contracts";

export type GamepadChannelConnection = "connected" | "reconnecting" | "disconnected";
...
function statesEqual(a: GamepadState, b: GamepadState): boolean {
  ...
  for (let i = 0; i < a.povs.length; i++) if (a.povs[i] !== b.povs[i]) return false;
  return true;
}

export function useGamepadChannel(workspaceSlug: string | null): UseGamepadChannelReturn {
  const [connection, setConnection] = useState<GamepadChannelConnection>("disconnected");
    ...
    const connect = () => {
      ...
      socket.addEventListener("open", () => {
        ...
        if (selection) {
          ...
          // we left off.
          socket.send(JSON.stringify({ type: "select", id: selection.id, label: selection.label }));
          seqRef.current = 0;
      ...
      socket.addEventListener("message", (event) => {
        if (!mountedRef.current) return;
        try {
          const msg = gamepadServerMessageSchema.parse(JSON.parse(String(event.data)));
