...
const VALID_STATUS = {
  ...
  halsim: { connection: "disconnected", connected: false, stale: false, lastMessageAt: null, error: null },
  driverStation: { enabled: false, mode: "teleop", eStopped: false, alliance: "blue1" },
  comms: { canEnable: false },
  joysticks: { status: "disconnected", port: null, label: null, lastInputAt: null },
};
...
describe("useSimulationState", () => {
  ...
  test("fetches status on mount", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith("/u/test-slug/api/sim/status", expect.objectContaining({ credentials: "same-origin" }));
  ...
  test("error response shows toast", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockImplementation((url: string) => {
        ...
        return Promise.resolve({ ok: true, json: () => Promise.resolve(VALID_STATUS) });
      }),
    );
  ...
  test("setDriverStation sends PATCH, updates local status on success", async () => {
    const updatedStatus = { ...VALID_STATUS, driverStation: { ...VALID_STATUS.driverStation, enabled: true } };
    vi.stubGlobal(
      "fetch",
      vi.fn().mockImplementation((url: string, opts?: RequestInit) => {
        if (url.includes("/api/sim/driver-station") && opts?.method === "PATCH") {
