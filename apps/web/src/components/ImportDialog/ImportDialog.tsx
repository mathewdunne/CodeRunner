	...
	return (
		...
		>
			<DialogContent
				...
				showCloseButton={state.status !== "importing"}
			>
				{step === "form" && (
					<>
						<DialogHeader>
							...
							<DialogDescription>
								Import a public GitHub repository as a snapshot. This replaces
						...
						<div className="grid gap-3 py-2">
							...
							<label className="flex items-center gap-2 text-xs">
								...
								<span className="text-muted-foreground">
									Back up current project before import
								</span>
							</label>
						</div>
											...
											>
												<div className="min-w-0 flex-1">
													<p className="truncate font-mono">
														{imp.url.replace("https://github.com/", "")}
													</p>
							...
							<ScrollArea className="max-h-32 rounded border bg-muted/30 p-2">
								<div className="space-y-0.5 font-mono text-[11px] text-muted-foreground">
									{state.logLines.map((line, i) => (
										// biome-ignore lint/suspicious/noArrayIndexKey: append-only log output, index is stable
										<p key={i}>{line}</p>
									))}
								</div>
							</ScrollArea>
