...
function preferredChooser(
	choosers: AutoChooser[],
...
export function AutoPanel({
	...
	const [selectedChooserKey, setSelectedChooserKey] = useState<string | null>(
		null,
	);
						...
						{chooser.options.map((option) => {
							...
							return (
								<button
									...
									onClick={() =>
										onSelectAuto({ key: chooser.key, selected: option })
									}
									className={cn(
