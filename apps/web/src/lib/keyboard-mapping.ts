...
const MAPPED_CODES: ReadonlySet<string> = new Set(
	KEYBOARD_BINDINGS.map((binding) => binding.code),
);
...
export function gamepadStateToVisualizerFrame(
	state: GamepadState,
): GamepadFrame {
	...
	for (let i = 0; i < 4; i++) {
		buttons[i] = {
			pressed: Boolean(state.buttons[i]),
			value: state.buttons[i] ? 1 : 0,
		};
	}
	buttons[4] = {
		pressed: Boolean(state.buttons[4]),
		value: state.buttons[4] ? 1 : 0,
	};
	buttons[5] = {
		pressed: Boolean(state.buttons[5]),
		value: state.buttons[5] ? 1 : 0,
	};
	buttons[6] = { pressed: (state.axes[2] ?? 0) > 0, value: state.axes[2] ?? 0 };
	buttons[7] = { pressed: (state.axes[3] ?? 0) > 0, value: state.axes[3] ?? 0 };
	buttons[8] = {
		pressed: Boolean(state.buttons[6]),
		value: state.buttons[6] ? 1 : 0,
	};
	buttons[9] = {
		pressed: Boolean(state.buttons[7]),
		value: state.buttons[7] ? 1 : 0,
	};
	buttons[10] = {
		pressed: Boolean(state.buttons[8]),
		value: state.buttons[8] ? 1 : 0,
	};
	buttons[11] = {
		pressed: Boolean(state.buttons[9]),
		value: state.buttons[9] ? 1 : 0,
	};
