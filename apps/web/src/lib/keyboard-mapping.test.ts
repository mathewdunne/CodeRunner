...
describe("keyboardCodesToWpilib", () => {
  test("maps left and right stick axes", () => {
    expect(keyboardCodesToWpilib(["KeyW", "KeyD", "ArrowUp", "ArrowRight"]).axes)
      .toEqual([1, -1, 0, 0, 1, -1]);
    expect(keyboardCodesToWpilib(["KeyA", "KeyS", "ArrowLeft", "ArrowDown"]).axes)
      .toEqual([-1, 1, 0, 0, -1, 1]);
  });

  test("cancels opposite axis directions", () => {
    expect(keyboardCodesToWpilib(["KeyA", "KeyD", "KeyW", "KeyS"]).axes)
      .toEqual([0, 0, 0, 0, 0, 0]);
    expect(keyboardCodesToWpilib(["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"]).axes)
      .toEqual([0, 0, 0, 0, 0, 0]);
  });

  test("maps triggers to WPILib trigger axes", () => {
    expect(keyboardCodesToWpilib(["KeyQ", "KeyO"]).axes)
      .toEqual([0, 0, 1, 1, 0, 0]);
  });

  test("maps buttons in WPILib XboxController order", () => {
    expect(keyboardCodesToWpilib([
      "KeyK",
      "KeyL",
      "KeyJ",
      "KeyI",
      ...
      "KeyF",
      "KeyH",
    ]).buttons).toEqual([true, true, true, true, true, true, true, true, true, true]);
  });
  ...
  test("keeps the exact Standard Xbox key assignments mapped", () => {
    ...
    expect(KEYBOARD_BINDINGS.map((binding) => binding.code)).toEqual(expectedCodes);
  });
});
