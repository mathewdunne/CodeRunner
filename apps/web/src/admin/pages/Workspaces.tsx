...
async function fetchWorkspaceData(): Promise<{ workspaces: WorkspaceStatus[]; usage: DiskUsage[] }> {
...
export function Workspaces() {
  const { data, loading, error, refetch } = useAdminPoll(useCallback(fetchWorkspaceData, []), 10000);
  const usageByWorkspace = useMemo(
    () => new Map((data?.usage ?? []).map((entry) => [entry.workspaceId, entry.bytes])),
    [data],
  );

  async function postWorkspaceAction(workspaceId: string, action: "backup" | "stop-containers" | "restart-code") {
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.workspaces.map((workspace) => (
                  <tr key={workspace.workspace.id} className="border-b last:border-0">
                    <td className="py-2 font-mono">{workspace.workspace.slug}</td>
                    <td className="py-2">
                      {workspace.user.displayName}
                      <div className="text-xs text-muted-foreground">{workspace.user.email}</div>
                    </td>
                    <td className="py-2">{new Date(workspace.workspace.lastAccessedAt).toLocaleString()}</td>
                    <td className="py-2">{formatBytes(usageByWorkspace.get(workspace.workspace.id) ?? 0)}</td>
                    ...
                    <td className="flex gap-2 py-2">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => void postWorkspaceAction(workspace.workspace.id, "backup")}
                      >
                        Backup
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => void postWorkspaceAction(workspace.workspace.id, "restart-code")}
                      >
                        Restart
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => void postWorkspaceAction(workspace.workspace.id, "stop-containers")}
                      >
                        Stop
                      </Button>
                    </td>
