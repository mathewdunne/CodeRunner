...
import { withApp, login, cookieFrom, createFakeDocker, workspaceBySlug } from "./helpers";
import {
  parseGitHubUrl,
  validateBranch,
  validateSubdir,
...
describe("parseGitHubUrl", () => {
  test("accepts simple GitHub HTTPS URL", () => {
    ...
    expect(result.cloneUrl).toBe("https://github.com/wpilibsuite/allwpilib.git");
    expect(result.branch).toBe("main");
    expect(result.subdir).toBe("");
  });

  test("accepts GitHub HTTPS URL with .git suffix", () => {
    const result = parseGitHubUrl("https://github.com/wpilibsuite/allwpilib.git");
    expect(result.cloneUrl).toBe("https://github.com/wpilibsuite/allwpilib.git");
    expect(result.branch).toBe("main");
    expect(result.subdir).toBe("");
  });
  ...
  test("parses tree URL into clone URL + branch + subdir", () => {
    const result = parseGitHubUrl("https://github.com/wpilibsuite/allwpilib/tree/main/wpilibjExamples");
    expect(result.cloneUrl).toBe("https://github.com/wpilibsuite/allwpilib.git");
    expect(result.branch).toBe("main");
  ...
  test("rejects SSH URL", () => {
    expect(() => parseGitHubUrl("git@github.com:owner/repo.git")).toThrow("SSH URLs are not supported");
  });

  test("rejects non-GitHub host", () => {
    expect(() => parseGitHubUrl("https://gitlab.com/owner/repo")).toThrow("Only GitHub URLs");
  });
  ...
  test("rejects unsupported GitHub path suffix", () => {
    expect(() => parseGitHubUrl("https://github.com/owner/repo/pulls")).toThrow("Unsupported GitHub URL format");
  });

  test("rejects subdir with '..'", () => {
    expect(() => parseGitHubUrl("https://github.com/owner/repo", undefined, "../escape")).toThrow("must not contain '..'");
  });
});
...
describe("import endpoint", () => {
  ...
  test("GET /api/project/recent-imports returns empty list initially", async () => {
    ...
    await withApp(async (app) => {
      ...
      const body = (await result.json()) as { ok: boolean; imports: unknown[] };
      expect(body.ok).toBe(true);
...
describe("restoreImportBackup", () => {
  test("rejects path traversal in archive file name", async () => {
    ...
    await withApp(async (app) => {
      ...
      await expect(restoreImportBackup(workspace, "../../../etc/passwd")).rejects.toThrow("Invalid backup file name");
      await expect(restoreImportBackup(workspace, "foo/bar.tar.gz")).rejects.toThrow("Invalid backup file name");
    }, { dockerRunner: docker.runner });
  });
  ...
  test("rejects missing backup file", async () => {
    ...
    await withApp(async (app) => {
      ...
      await expect(restoreImportBackup(workspace, "nonexistent.tar.gz")).rejects.toThrow("Backup file not found");
    }, { dockerRunner: docker.runner });
  });
});
