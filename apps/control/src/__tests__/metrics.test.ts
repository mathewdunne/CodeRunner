...
describe("templateRoute", () => {
  ...
  test("collapses prefix-shared paths", () => {
    ...
    expect(templateRoute("/scope/bundled/Crescendo/field.json")).toBe("/scope/*");
  ...
  test("strips workspace slug to :slug", () => {
    ...
    expect(templateRoute("/u/student-42/api/sim/status")).toBe("/u/:slug/api/sim/status");
  ...
  test("collapses vscode and assets subtrees inside workspace", () => {
    ...
    expect(templateRoute("/u/jane/vscode/static/out/vs/loader.js")).toBe("/u/:slug/vscode/*");
    expect(templateRoute("/u/jane/assets/index-abc.css")).toBe("/u/:slug/assets/*");
  });
