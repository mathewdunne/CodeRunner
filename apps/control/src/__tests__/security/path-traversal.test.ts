...
describe("S7 — restoreImportBackup rejects path traversal in archiveFile", () => {
	test("`..` in archive file name rejected", async () => {
		const root = await mkdir(join(tmpdir(), `frc-restore-${Date.now()}`), {
			recursive: true,
		});
		...
		try {
			await expect(
				restoreImportBackup(workspace, "../escape.tar.gz"),
	...
	test("slash in archive file name rejected", async () => {
		...
		try {
			await expect(
				restoreImportBackup(workspace, "subdir/file.tar.gz"),
			).rejects.toThrow(ImportError);
		} finally {
			await rm(root!, { recursive: true, force: true });
		}
	...
	test("non-existent backup name rejected with typed error", async () => {
		...
		try {
			await expect(
				restoreImportBackup(workspace, "nope.tar.gz"),
			).rejects.toThrow(ImportError);
		} finally {
			await rm(root!, { recursive: true, force: true });
		}
