...
import { withApp, login, cookieFrom, workspaceBySlug } from "./helpers";

/**
 * Build a minimal valid AdvantageScope asset ZIP containing a single directory
 * with a config.json and one model file.
 */
async function createAssetZip(root: string, assetName: string, config: object = { name: assetName }): Promise<string> {
  const assetDir = join(root, assetName);
  await mkdir(assetDir, { recursive: true });
  await writeFile(join(assetDir, "config.json"), JSON.stringify(config), "utf8");
  await writeFile(join(assetDir, "model.glb"), "fake-model-data", "utf8");

  const zipPath = join(root, `${assetName}.zip`);
  const proc = Bun.spawn(["zip", "-r", zipPath, assetName], { cwd: root, stdout: "pipe", stderr: "pipe" });
  await proc.exited;
  return zipPath;
}
...
describe("AdvantageScope upload asset", () => {
  ...
  test("POST /scope/uploadAsset with valid ZIP extracts to user assets dir", async () => {
    await withApp(async (app, root) => {
      ...
      const zipPath = await createAssetZip(join(root, "zips"), "Robot_Custom", { name: "Robot_Custom", type: "Robot" });
      const zipFile = Bun.file(zipPath);

      const formData = new FormData();
      formData.append("file", new File([await zipFile.arrayBuffer()], "Robot_Custom.zip"));
      ...
      const config = JSON.parse(await readFile(join(assetsDir, "Robot_Custom", "config.json"), "utf8"));
  ...
  test("POST /scope/uploadAsset rejects ZIP without config.json", async () => {
    await withApp(async (app, root) => {
      ...
      const proc = Bun.spawn(["zip", "-r", zipPath, "BadAsset"], { cwd: join(root, "zips"), stdout: "pipe", stderr: "pipe" });
      await proc.exited;

      const formData = new FormData();
      formData.append("file", new File([await Bun.file(zipPath).arrayBuffer()], "BadAsset.zip"));
  ...
  test("GET /scope/assets manifest includes user-uploaded assets merged with bundled", async () => {
    await withApp(async (app, root) => {
      ...
      const userAssetDir = join(dirname(workspace.project_path), "assets", "Field2d_Custom");
      await mkdir(userAssetDir, { recursive: true });
      await writeFile(join(userAssetDir, "config.json"), JSON.stringify({ name: "Field2d_Custom" }), "utf8");
  ...
  test("GET /scope/assets/<user-asset-path> serves files from user assets dir", async () => {
    await withApp(async (app, root) => {
      ...
      // Place a user asset
      const userAssetDir = join(dirname(workspace.project_path), "assets", "Robot_Custom");
      await mkdir(userAssetDir, { recursive: true });
      await writeFile(join(userAssetDir, "config.json"), JSON.stringify({ name: "Robot_Custom" }), "utf8");
