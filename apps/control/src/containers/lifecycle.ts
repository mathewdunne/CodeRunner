...
export async function stopWorkspaceSim(
  ...
  const result = await runDocker(dockerRunner, ["exec", name, "/usr/local/bin/stop-sim.sh"], true);
...
export async function countRunningContainers(dockerRunner: DockerRunner): Promise<number> {
...
export async function cleanupStoppedContainers(dockerRunner: DockerRunner): Promise<string[]> {
  const result = await runDocker(
    dockerRunner,
    [
      "container",
