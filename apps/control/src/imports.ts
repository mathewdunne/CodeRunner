...
export function parseGitHubUrl(
  ...
  if (/^git@/i.test(url)) {
    throw new ImportError("SSH URLs are not supported. Use an HTTPS GitHub URL.");
  }
...
export class ImportManager {
  ...
  private async executeImport(ctx: ImportContext, importId: string): Promise<void> {
    ...
    try {
      ...
      if (checkResult.exitCode !== 0) {
        throw new ImportError("Not a Gradle/WPILib project: build.gradle not found at the project root.");
      }
      ...
      if (backup) {
        send({ type: "progress", stage: "backup", detail: "Backing up current project…" });
        await this.backupProject(workspace, cloneUrl, branch, subdir, timestamp);
  ...
  private async backupProject(
    ...
    // Create the archive
    const subprocess = Bun.spawn(["tar", "-czf", archivePath, "-C", projectDir, "."], {
      stdout: "pipe",
      stderr: "pipe",
    });
    ...
    if (exitCode !== 0) {
      throw new ImportError(`Backup failed: ${stderr.trim() || `exit ${exitCode}`}`);
    }
  ...
  private async pruneImportBackups(backupsDir: string): Promise<void> {
    ...
    const toRemove = metadataFiles.slice(0, metadataFiles.length - MAX_IMPORT_BACKUPS);
...
export async function restoreImportBackup(
  ...
  try {
    ...
    if (exitCode !== 0) {
      throw new ImportError(`Restore failed: ${stderr.trim() || `exit ${exitCode}`}`);
    }
    ...
    for (const entry of restoredEntries) {
      ...
      await cp(resolve(tempDir, entry), resolve(projectDir, entry), { recursive: true });
    }
  } finally {
