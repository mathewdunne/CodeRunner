...
export function verifyAppliedMigrationChecksums(
	db: Database,
	...
	for (const applied of listAppliedMigrations(db)) {
		...
		if (!migration) {
			throw new Error(
				`Applied migration ${applied.name} is missing from disk.`,
			);
		}
...
export async function migrationStatus(
	...
	const appliedByName = new Map(
		listAppliedMigrations(db).map((row) => [row.name, row]),
	);
...
export async function applyMigrations(
	...
	const appliedNames = new Set(
		listAppliedMigrations(db).map((row) => row.name),
	);
