...
export type Nt4AutoWebSocketFactory = (
	url: string,
	protocols: string[],
) => WebSocket;
...
function upstreamUrlFor(value: string | number): string {
	return typeof value === "number"
		? `ws://127.0.0.1:${value}/nt/${APP_NAME}`
		: value;
}
...
function readString(
	...
	return (
		stringValue(entry.valuesByName.get(`${root}/${field}`)) ??
		stringValue(entry.valuesByName.get(`${root}/.${field}`))
	);
}
...
function readOptions(entry: Nt4AutoEntry, root: string): string[] {
	...
	const count =
		typeof length === "number" ? Math.max(0, Math.floor(length)) : 0;
...
function isChooser(entry: Nt4AutoEntry, root: string): boolean {
	...
	return (
		readOptions(entry, root).length > 0 &&
		(readString(entry, root, "default") !== null ||
			readString(entry, root, "active") !== null)
	);
}
...
function encodeMsgPack(value: unknown): Uint8Array {
	...
	const write = (item: unknown) => {
		if (Array.isArray(item)) {
			...
			else if (Number.isInteger(item) && item >= 0 && item <= 0xffffffff)
				push(0xce, item >>> 24, item >>> 16, item >>> 8, item);
...
export class Nt4AutoChooserBridge {
	...
	select(
		...
		if (!entry?.socket || entry.socket.readyState !== WebSocket.OPEN) {
			throw new Nt4AutoChooserUnavailableError(
				snapshot.nt4.error ?? "NT4 auto chooser bridge is not connected.",
			);
		}
		const chooser = snapshot.choosers.find(
			(candidate) => candidate.key === patch.key,
		);
		if (!chooser) {
		...
		entry.socket.send(
			encodeMsgPack([
				entry.publishedTopics.get(topicName)?.id,
				Date.now() * 1000,
				STRING_TYPE_INDEX,
				patch.selected,
			]),
		...
		try {
			...
			entry.error =
				error instanceof Error
					? error.message
					: "Unable to decode NT4 binary frame.";
			return;
		}
	...
	private publishStringTopic(entry: Nt4AutoEntry, name: string): void {
		...
		const topic = {
			id: Math.floor(Math.random() * 99_999_999),
			name,
			type: "string",
		};
