...
export async function startFakeNt4(options: FakeNt4Options = {}): Promise<FakeNt4Handle> {
  ...
  function checkWaiters() {
    ...
    connectionWaiters = connectionWaiters.filter((w) => connectionCount < w.target);
  }
    ...
    awaitConnection(n?: number, timeout = 5000): Promise<void> {
      ...
      return new Promise<void>((resolve, reject) => {
        const timer = setTimeout(() => {
          ...
          reject(new Error(`Timed out waiting for NT4 connection #${target} (got ${connectionCount})`));
        }, timeout);
