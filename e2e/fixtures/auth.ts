...
export type LoginResult = {
  user: { id: string; email: string; name: string; slug: string; role: "student" | "admin" };
  cookieValue: string;
...
export async function loginAs(
  ...
  const email = (opts.email ?? `${opts.name.toLowerCase()}@test.local`).toLowerCase();
  const slug = opts.name.toLowerCase().replace(/[^a-z0-9_-]/g, "-").slice(0, 40);
  ...
  if (existing) {
    userId = existing.id;
    db.query("UPDATE user SET role = ?, updatedAt = ? WHERE id = ?").run(role, now, userId);
  } else {
    userId = randomToken();
    db.query(
