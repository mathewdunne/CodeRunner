  ...
  await page.addInitScript(() => {
    ...
    (window as unknown as Record<string, unknown>).__gamepadShim = {
      ...
      setButton(index: number, btnIndex: number, pressed: boolean) {
        if (pads[index]) {
          const buttons = (pads[index] as unknown as { buttons: GamepadButton[] }).buttons;
...
export async function connectGamepad(page: Page, index = 0, id?: string): Promise<void> {
  await page.evaluate(
    ({ index, id }) => {
...
export async function disconnectGamepad(page: Page, index: number): Promise<void> {
...
export async function setGamepadAxes(page: Page, index: number, axes: number[]): Promise<void> {
  await page.evaluate(
    ({ index, axes }) => {
      (window as unknown as Record<string, any>).__gamepadShim.setAxes(index, axes);
    },
    { index, axes },
  );
}
...
export async function setGamepadButton(
  ...
  await page.evaluate(
    ({ index, btnIndex, pressed }) => {
      (window as unknown as Record<string, any>).__gamepadShim.setButton(index, btnIndex, pressed);
    },
    { index, btnIndex, pressed },
  );
}
