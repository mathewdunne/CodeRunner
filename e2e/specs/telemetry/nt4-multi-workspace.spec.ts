...
test("NT4 traffic isolated per workspace", async ({ app, page, runtime, fakeVscode, fakeHalsim }) => {
  ...
  try {
    ...
    const aliceReady = new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("Alice got no NT4 messages")), 5000);
    ...
    const bobReady = new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("Bob got no NT4 messages")), 5000);
    ...
    expect(aliceMessages).toEqual([
      { method: "announce", params: { name: "/Robot/A", type: "string", id: 1 } },
    ]);

    // Bob should only see /Robot/B topics
    expect(bobMessages).toEqual([
      { method: "announce", params: { name: "/Robot/B", type: "string", id: 2 } },
    ]);
...
test("NT4 proxy rejects cross-workspace access", async ({ app, page, runtime, fakeVscode, fakeHalsim }) => {
