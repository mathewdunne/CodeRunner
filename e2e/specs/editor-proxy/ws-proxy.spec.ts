...
test("editor WebSocket connects through proxy", async ({
  ...
  const helloPromise = new Promise<unknown>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error("No hello frame received")), 5000);
  ...
  const echoPromise = new Promise<unknown>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error("No echo frame received")), 5000);
    ws.addEventListener("message", (event) => {
      clearTimeout(timer);
      resolve(JSON.parse(event.data as string));
    }, { once: true });
...
test("editor WS reconnects cleanly after refresh", async ({
  ...
  const helloPromise = new Promise<unknown>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error("No hello on reconnect")), 5000);
  ...
  expect(consoleErrors.filter((e) => /protocol|websocket/i.test(e))).toHaveLength(0);
