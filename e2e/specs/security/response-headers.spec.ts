...
test("login response sets HttpOnly + SameSite + Path on session cookie", async ({ app }) => {
...
test("/healthz response does not include sensitive cache headers", async ({ app }) => {
  const resp = await app.fetch(new Request(`${app.storage.config.baseUrl}/healthz`));
  // We don't want healthz to set Set-Cookie or expose internal version strings
  // beyond what the contract documents.
  expect(resp.headers.get("set-cookie")).toBeNull();
});

test("X-Content-Type-Options nosniff is present on text/* responses (when configured)", async ({ app }) => {
  const baseUrl = app.storage.config.baseUrl;
